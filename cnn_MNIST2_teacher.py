...
def cnn_model_fn_teacher(features, labels,params, mode):
  ...
  logits = tf.layers.dense(inputs=dropout, units=358)
  predictions = {
  ...
  loss_train = tf.losses.sparse_softmax_cross_entropy(labels=labels, logits=tf.divide(logits,temperature))
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    return tf.estimator.EstimatorSpec(mode=mode, loss=loss_train, train_op=train_op)
...
def trainModel(trainSamples,trainLabels,testSamples,testLabels):
  ...
  mnist_classifier = tf.estimator.Estimator(
      model_fn=cnn_model_fn, params = {"temperature": 2}, model_dir="/tmp/mnist_convnet_model5")
  ...
  soft_targets = predModel(trainSamples)
